...
export default async function PracticePage() {
  ...
  const { data } = await supabase
    .from('words')
    .select(
      'id, term, reading, meaning_id, meaning_en, example_jp, example_furigana, example_translation, jlpt, part_of_speech, ease_factor, interval, repetitions, due_date, sources ( name, detail, type )',
    )
  ...
  const words = (data ?? []).map((w) => {
    ...
    return {
      ...
      example_furigana: w.example_furigana,
